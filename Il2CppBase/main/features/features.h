...
namespace Il2CppBase
{
	...
	// Replace Il2CppBase on your cheat name
...
#define GET_FEATURE_HANDLER() (::Il2CppBase::MainFeatureHandler)

#define FEATURE_ENABLED(Feature) \
        (Feature->IsActive())

#define CALL_METHOD(Category, FeatureName, MethodName) \
    GET_FEATURE_HANDLER()->GetFeature<Il2CppBase::Features::Category::FeatureName>(#FeatureName)->MethodName()

#define CALL_METHOD_IF_ACTIVE(Category, FeatureName, MethodName) \
    if (auto* feature = GET_FEATURE_HANDLER()->GetFeature<Il2CppBase::Features::Category::FeatureName>(#FeatureName); feature && feature->IsActive()) \
        feature->MethodName()
...
#define CALL_METHOD_ARGS(Category, FeatureName, MethodName, ...) \
    GET_FEATURE_HANDLER()->GetFeature<Il2CppBase::Features::Category::FeatureName>(#FeatureName)->MethodName(__VA_ARGS__)

#define GET_ACTIVE(Category, FeatureName) \
    (GET_FEATURE_HANDLER()->GetFeature<Il2CppBase::Features::Category::FeatureName>(#FeatureName) && \
     GET_FEATURE_HANDLER()->GetFeature<Il2CppBase::Features::Category::FeatureName>(#FeatureName)->IsActive())
